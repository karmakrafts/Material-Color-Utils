...
package io.karma.material.contrast;

import static java.lang.Math.max;

import io.karma.material.utils.ColorUtils;

/**
 * Color science for io.karma.material.contrast utilities.
 *
 * <p>Utility methods for calculating io.karma.material.contrast given two colors, or calculating a color given one
 * color and a io.karma.material.contrast ratio.
 *
...
public final class Contrast {
  // The minimum io.karma.material.contrast ratio of two colors.
  // Contrast ratio equation = lighter + 5 / darker + 5, if lighter == darker, ratio == 1.
  public static final double RATIO_MIN = 1.0;

  // The maximum io.karma.material.contrast ratio of two colors.
  ...
  // Given a color and a io.karma.material.contrast ratio to reach, the luminance of a color that reaches that ratio
  // with the color can be calculated. However, that luminance may not io.karma.material.contrast as desired, i.e. the
  // io.karma.material.contrast ratio of the input color and the returned luminance may not reach the io.karma.material.contrast ratio
  // asked for.
  //
  // When the desired io.karma.material.contrast ratio and the result io.karma.material.contrast ratio differ by more than this amount,
  // an error value should be returned, or the method should be documented as 'unsafe', meaning,
  // it will return a valid luminance but that luminance may not meet the requested io.karma.material.contrast ratio.
  //
  ...
  //
  // A principled solution is to maintain lightness, thus maintaining io.karma.material.contrast/a11y, maintain hue,
  ...
  // display color spaces, methods that take a io.karma.material.contrast ratio and luminance, and return a luminance
  // that reaches that io.karma.material.contrast ratio for the input luminance, purposefully darken/lighten their
  // result such that the desired io.karma.material.contrast ratio will be reached even if inaccuracy is introduced.
  //
  ...
  // lightness rounds to the same as the requested, the desired io.karma.material.contrast ratio will be reached.
   ...
   * spaces, and measure io.karma.material.contrast, and measure io.karma.material.contrast in a much more understandable way: instead
   * of a ratio, a linear difference. This allows a designer to determine what they need to adjust a
   * color's lightness to in order to reach their desired io.karma.material.contrast, instead of guessing & checking
   * with hex codes.
   */
   ...
   *
   * @param tone Tone return value must io.karma.material.contrast with.
   * @param ratio Desired io.karma.material.contrast ratio of return value and tone parameter.
   */
  public static double lighter(double tone, double ratio) {
    if (tone < 0.0 || tone > 100.0) {
      return -1.0;
    }
    // Invert the io.karma.material.contrast ratio equation to determine lighter Y given a ratio and darker Y.
  ...
  public static double darker(double tone, double ratio) {
    ...
    // Invert the io.karma.material.contrast ratio equation to determine darker Y given a ratio and lighter Y.
